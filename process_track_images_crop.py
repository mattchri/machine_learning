...
# in the satellite granule
#
# Procedure
# 1) Plot 2.1 um NIR image for each MODIS granule in the OSU shiptrack
# database.
# 2) Plot 2.1 um NIR image with hand-logged track positions
# 3) Plot global distribution of ship track hand-logged positions
#
#Example
#python2.7 -i process_track_images_crop.py
#
# 10/02/18, MC: upload initial version of the code to the repo
#---------------------------------------------------------------

#---------------------------------------------------------------
# Libraries
#---------------------------------------------------------------
from subroutines_track_images_crop import *
import datetime
import numpy as np
import os,sys,glob
...
for i in range(fct):
    ...
    if len(file02) > 0 and len(file03) > 0 and len(file04) > 0 and len(file06) > 0:
        ...
        # Read MODIS Attribute Data (size of image)
        ...
        #Calculate bounding boxes for machine learning code
        trainData = track_to_bbox(track_points,xN,yN,cropped_area)

        #Extract all flags from trainData
        flags = []
...
#tarball tar -czvf name-of-archive.tar.gz /path/to/directory-or-file
